...
/*static JSStaticFunction jsFunctions[] =
{
 {"async", JsProxy::asyncFunction, kJSPropertyAttributeNone},
 {"sync", JsProxy::syncFunction, kJSPropertyAttributeNone},
 {0,0,0}
};

JSValueRef JsProxy::asyncFunction(JSContextRef ctx, JSObjectRef function, JSObjectRef thisObject, size_t argumentCount,
...
JSValueRef JsProxy::syncFunction(JSContextRef ctx, JSObjectRef function,
    ...
    LOGD("size of map = %d", list.size());
    std::map<char*, NativeModule*>::iterator iter;
    for (iter = list.begin(); iter != list.end(); iter++) {
        LOGD("get module name ======== %s", iter->first);
        ...
        for (p = params; p != NULL && i < len; p++, i++) {
             LOGD("methods name === %s %s", (*p)->name, (*p)->sign);
        }
    }

    JSStringRef ref = JSStringCreateWithUTF8CString("syncFunction returned values");
    return JSValueMakeString(ctx, ref);
}

static JSClassDefinition jsProxyDefinition =
{
    0,                     //version
    kJSClassAttributeNone, //attributes
    "__JsProxyClass",       //className
    0,                     //parentClass
    0,                     //staticValues
    jsFunctions,     //staticFunctions
    0,                     //Initialize
    0,                     //Finalize
    0,                     //has Property
    0,                     //get Property
